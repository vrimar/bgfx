...
#define $A D3D12_SHADER_COMPONENT_MAPPING_FROM_MEMORY_COMPONENT_3
		...
		{ DXGI_FORMAT_R24G8_TYPELESS,     DXGI_FORMAT_R24_UNORM_X8_TYPELESS,    DXGI_FORMAT_UNKNOWN,              DXGI_FORMAT_UNKNOWN,               D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING             }, // D24
		...
		{ DXGI_FORMAT_R24G8_TYPELESS,     DXGI_FORMAT_R24_UNORM_X8_TYPELESS,    DXGI_FORMAT_UNKNOWN,              DXGI_FORMAT_UNKNOWN,               D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING             }, // D32
		{ DXGI_FORMAT_R32_TYPELESS,       DXGI_FORMAT_R32_FLOAT,                DXGI_FORMAT_UNKNOWN,              DXGI_FORMAT_UNKNOWN,               D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING             }, // D16F
		{ DXGI_FORMAT_R32_TYPELESS,       DXGI_FORMAT_R32_FLOAT,                DXGI_FORMAT_UNKNOWN,              DXGI_FORMAT_UNKNOWN,               D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING             }, // D24F
...
#undef $A
	...
	static const D3D12_INPUT_ELEMENT_DESC s_attrib[] =
	{
		...
		{ "TEXCOORD",     9, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    10, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    11, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    12, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    13, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    14, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
		{ "TEXCOORD",    15, DXGI_FORMAT_R32G32_FLOAT,    0, D3D12_APPEND_ALIGNED_ELEMENT, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
	};
...
#	endif // !BX_PLATFORM_LINUX
		...
		static bool hasStencilPlane(TextureFormat::Enum _format)
		{
			return false
		...
		void readTexture(TextureHandle _handle, void* _data, uint16_t _layer, uint8_t _mip) override
		{
			...
			if (hasStencilPlane(TextureFormat::Enum(texture.m_textureFormat) ) )
			{
				readTexturePlanar(texture, desc, subresource, _data, _mip);
				return;
			}
		...
		void readTexturePlanar(TextureD3D12& _texture, const D3D12_RESOURCE_DESC& _desc, uint32_t _subresource, void* _data, uint8_t _mip)
		{
			...
			m_device->GetCopyableFootprints(&_desc
				, _subresource
				, 1
				, 0
				, &layout[0]
			...
			const uint64_t stencilOffset = bx::alignUp<uint64_t>(total[0], D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);

			m_device->GetCopyableFootprints(&_desc
				, _subresource + numSubresourcesPerPlane
				, 1
				, stencilOffset
				, &layout[1]
				, &numRows[1]
			...
			for (uint32_t plane = 0; plane < BX_COUNTOF(layout); ++plane)
			{
				...
				D3D12_TEXTURE_COPY_LOCATION srcLocation = { _texture.m_ptr, D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX, { }               };
...
#	endif // BX_PLATFORM_WINDOWS || BX_PLATFORM_WINRT
		...
		uint32_t setInputLayout(D3D12_INPUT_ELEMENT_DESC* _vertexElements, uint8_t _numStreams, const VertexLayout** _layouts, const ProgramD3D12& _program, uint16_t _numInstanceData)
		{
			...
			{
				uint32_t index = BGFX_CONFIG_INSTANCE_DATA_FIRST_TEXCOORD - ii; // TEXCOORD(x) = i_data0, TEXCOORD(x-1) = i_data1, etc.
	...
	{
		create(_chunkSize, _numChunks, D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
	}

	void ChunkedScratchBufferD3D12::createChunk(ChunkD3D12& _chunk)
	{
		ID3D12Device* device = s_renderD3D12->m_device;

		_chunk.upload = createCommittedResource(device, HeapProperty::Upload, m_chunkSize);
		_chunk.buffer = _chunk.upload->GetGPUVirtualAddress();

		D3D12_RANGE readRange = { 0, 0 };
		_chunk.upload->Map(0, &readRange, (void**)&_chunk.data);
	}

	void ChunkedScratchBufferD3D12::destroyChunk(ChunkD3D12& _chunk)
	{
		D3D12_RANGE writeRange = { 0, 0 };
		_chunk.upload->Unmap(0, &writeRange);

		DX_RELEASE(_chunk.upload, 0);
	}

	void ChunkedScratchBufferD3D12::flushChunk(ChunkD3D12& _chunk, uint32_t _size)
	{
		// Upload heap is CPU-coherent and persistently mapped; nothing to flush.
		BX_UNUSED(_chunk, _size);
	}

	uint32_t ChunkedScratchBufferD3D12::currentFrameInFlight() const
	{
		return s_renderD3D12->m_backBufferColorIdx;
	}
	...
	void* TextureD3D12::create(const Memory* _mem, uint64_t _flags, uint8_t _skip, uint64_t _external)
	{
		...
		if (bimg::imageParse(imageContainer, _mem->data, _mem->size) )
		{
			...
			const bool computeWrite   = false
				|| 0 != (m_flags & BGFX_TEXTURE_COMPUTE_WRITE)
				|| (renderTarget && 1 < ti.numMips && !bimg::isDepth(bimg::TextureFormat::Enum(m_textureFormat) ) )
				|| isVideoDecodeDst
				;
...
//				&& !readBack
	...
	void TextureD3D12::update(ID3D12GraphicsCommandList* _commandList, uint8_t _side, uint8_t _mip, const Rect& _rect, uint16_t _z, uint16_t _depth, uint16_t _pitch, const Memory* _mem)
	{
		...
		if (convert)
		{
			...
			box.bottom = bx::clamp<uint32_t>(box.bottom, box.top,  bx::max(1u, m_height >> _mip) - _rect.m_y);
		}
	...
	bool TextureD3D12::isMsaaSurface() const
	{
		const uint32_t msaaQuality = bx::satSub<uint32_t>(uint32_t( (m_flags&BGFX_TEXTURE_RT_MSAA_MASK) >> BGFX_TEXTURE_RT_MSAA_SHIFT), 1u);
		return true
...
// 							| (blockInfo.stencilBits > 0 ? D3D12_DSV_FLAG_READ_ONLY_STENCIL : D3D12_DSV_FLAG_NONE)
	...
	bool RendererContextD3D12::blitBufferTextureCompute(const BlitItem& _blit)
	{
		...
		m_commandList->Dispatch(
			  bx::max<uint32_t>( (_blit.m_width  + 7) / 8, 1)
			, bx::max<uint32_t>( (_blit.m_height + 7) / 8, 1)
			, depth
			);
		...
		for (uint32_t slice = 0; slice < numSlices; ++slice)
		{
			const uint32_t subresource = is3D
				...
				: (TextureD3D12::TextureCube == texture.m_type
					? z + slice
					: bx::min<uint32_t>(z + slice, texture.m_numLayers - 1)
	...
	void RendererContextD3D12::submitBlit(BlitState& _bs, uint16_t _view)
	{
		...
		while (_bs.hasItem(_view) )
		{
			...
			&&  !dst.isMsaaSurface()
			&&  TextureD3D12::Texture3D != src.m_type
			&&  kInvalidHandle != dstIdx)
			{
				const TextureFormatInfo& tfi = s_textureFormat[src.m_textureFormat];
				const DXGI_FORMAT resolveFormat = bimg::isDepth(bimg::TextureFormat::Enum(src.m_textureFormat) )
					? tfi.m_fmt
					: (0 != (src.m_flags & BGFX_TEXTURE_SRGB) ? tfi.m_fmtSrgb : tfi.m_fmtSrv)
					;

				src.setState(m_commandList, D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
				dst.setState(m_commandList, D3D12_RESOURCE_STATE_RESOLVE_DEST);

				m_commandList->ResolveSubresource(
					  dst.m_ptr
					, blit.m_dstZ*dst.m_numMips + blit.m_dstMip
					, src.m_ptr
					, blit.m_srcZ*src.m_numMips + blit.m_srcMip
					, resolveFormat
					);

				src.setState(m_commandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
				dst.setState(m_commandList, D3D12_RESOURCE_STATE_COPY_DEST);
				continue;
			}
			...
			if (TextureD3D12::Texture3D == src.m_type)
			{
				...
				const D3D12_TEXTURE_COPY_LOCATION dstLocation =
				{
					.pResource        = dst.m_ptr,
					.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
					.SubresourceIndex = blit.m_dstMip,
				};
				...
				const D3D12_TEXTURE_COPY_LOCATION srcLocation =
				{
					.pResource        = src.m_ptr,
					.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
					.SubresourceIndex = blit.m_srcMip,
				};
			...
			else
			{
				...
				const D3D12_TEXTURE_COPY_LOCATION dstLocation =
				{
					.pResource        = dst.m_ptr,
					.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
					.SubresourceIndex = dstZ*dst.m_numMips+blit.m_dstMip,
				};
				...
				if (hasStencilPlane(TextureFormat::Enum(src.m_textureFormat) )
				&&  hasStencilPlane(TextureFormat::Enum(dst.m_textureFormat) ) )
				{
					const D3D12_TEXTURE_COPY_LOCATION dstStencilLocation =
					{
						.pResource        = dstLocation.pResource,
						.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
						.SubresourceIndex = dstLocation.SubresourceIndex + dst.m_numMips*getResourceDesc(dstLocation.pResource).DepthOrArraySize,
					};

					const D3D12_TEXTURE_COPY_LOCATION srcStencilLocation =
					{
						.pResource        = srcLocation.pResource,
						.Type             = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX,
						.SubresourceIndex = srcLocation.SubresourceIndex + src.m_numMips*getResourceDesc(srcLocation.pResource).DepthOrArraySize,
					};
